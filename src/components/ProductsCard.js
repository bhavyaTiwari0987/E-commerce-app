...
const ProductsCard = ({ product }) => {
  ...
  return (
    <div className="group relative">
      <div onClick={handleDetails} className="w-full h-72 cursor-pointer overflow-hidden">
        <img
          className="w-full h-fit object-cover group-hover:scale-110 duration-500"
          src={product.image}
          alt="productImg"
        />
      </div>
      <div className="w-full border-[1px] px-2 py-4">
        ...
        <div className="absolute top-4 right-0">{product.rating.count > 300 && <p className="bg-black text-white font-semibold font-titleFont px-6 py-1">Sale</p>}</div>
      </div>
      <ToastContainer
        position="top-right"
        autoClose={5000}
